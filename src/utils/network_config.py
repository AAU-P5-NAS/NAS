...
def get_layer_from_index(observation: np.ndarray, index: int, max_layers: int) -> LayerConfig:
    ...
    return LayerConfig.from_latest_observation(
        observation[start : start + SINGLE_LAYER_OBSERVATION_SIZE]
    )
...
def get_observation_with_new_layer(observation, ctx: "MaskContext"):
    ...
    new_layer_flattened = np.array(
        flatten_cnn_config(
            NetworkConfig(layers=[new_layer]), ctx.max_layers, padded_with_zeros=False
        ),
        dtype=np.float32,
    )
    ...
    observation[
        last_layer_index * SINGLE_LAYER_OBSERVATION_SIZE : (last_layer_index + 1)
...
def get_latest_layer(
    ...
    out_channels = (
        OutChannels(int(observation[idx + 1])) if observation[idx + 1] != 0 else OutChannels.NONE
    )
    kernel_size = (
        KernelSize(int(observation[idx + 2])) if observation[idx + 2] != 0 else KernelSize.NONE
    )
    ...
    activation = (
        ActivationFunction(int(observation[idx + 5]))
